...
const Cart = () => {
  const { stepNumber, setStep, isFormValid, clearOrder, order, setOnFormSubmit, onFormSubmit } = useCart();

  const renderButton = () => {
    let disabled = false;
    let onClick;
    const width = "calc(50% - 3px)";
    ...
    switch (stepNumber) {
      ...
      case 1: {
        ...
        onClick = () => {setOnFormSubmit(!onFormSubmit)
              ...
              <Stack>
                ...
                <ButtonGroup size="sm" variant="outline" gap="6px" justifyContent="space-between">
                  <Steps.PrevTrigger asChild>
                    {stepNumber === 0 
                    ? <Button asChild width="calc(50% - 3px)" disabled={false}>
                        <Link to="/">Назад</Link>
                      </Button>
                    : <Button width="calc(50% - 3px)">Назад</Button>
                    }
                  </Steps.PrevTrigger>
